...
import { APPROVAL_URL, APP_STATE, CHAINS_ENUMS, CHAINS_IDS } from "../../../../constants";
import {
    BROADCAST_REQUEST,
    COSMOS_REQUESTS,
    ETHEREUM_REQUESTS,
...
const flowContext = flow
    .use(async (ctx) => {
        ...
        if (
            ...
            const solanaWalletDetail = currentWallet.accounts.find(
                (wall) => wall.chain === CHAINS_IDS.SOLANA,
            );
            const suiWalletDetail = currentWallet.accounts.find(
                (wall) => wall.chain === CHAINS_IDS.SUI,
            );
            return new Promise((resolve, reject) => {
                if (walletController.siteApproved(origin)) {
                    walletController?.getSelectedWallet().then(async (res) => {
                        if (res) {
                            if (method === SOLANA_REQUESTS.requestSolAccount) {
                                sessionService.broadcastEvent(
                                    BROADCAST_REQUEST.connect,
                                    solanaWalletDetail.address,
                                );
                                resolve([solanaWalletDetail.address]);
                            } else if (method === COSMOS_REQUESTS.requestCosAccount) {
                                if (!walletController.isUnlocked()) {
                                    notificationService
                                        ...
                                        .then(async (res) => {
                                            const pubKey =
                                                await walletController.getCosmosKeyDetails(
                                                    params,
                                                );
                                            const chainDetail =
                                                currentWallet.accounts.find(
                                                    (acc) =>
                                                        acc.chainId === params.chainId,
                                                );
                                            resolve([
                                                {
                                                    address:
                                                        chainDetail?.address?.toLowerCase(),
                                                    pubKey: pubKey,
                                                },
                                            ]);
                                        })
                                    ...
                                    const chainDetail = currentWallet?.accounts.find(
                                        (acc) => acc.chainId === params.chainId,
                                    );
                                    resolve([
                                        {
                                            address: chainDetail?.address?.toLowerCase(),
                                            pubKey: pubKey,
                                        },
                                    ]);
                                }
                            } else if (method === SUI_REQUESTS.requestSuiAccount) {
                                if (!walletController.isUnlocked()) {
                                    notificationService
                                        .requestApproval(
                                            {
                                        ...
                                        .then(async (res) => {
                                            const pubKey =
                                                await walletController.getSuiKeyDetails();
                                            sessionService.broadcastEvent(
                                                BROADCAST_REQUEST.connect,
                                                suiWalletDetail.address,
                                            );
                                            resolve({
                                                address: suiWalletDetail.address,
                                                publicKey: pubKey,
                                                method: "connected",
                                                status: 200,
                                            });
                                ...
                                resolve([
                                    currentWallet.selectedChain.address?.toLowerCase(),
                                ]);
                            }
                        }
                    });
                    ...
                    notificationService
                        ...
                        .then(async (res) => {
                            ...
                            if (SOLANA_REQUESTS.requestSolAccount === method) {
                                sessionService.broadcastEvent(
                                    BROADCAST_REQUEST.connect,
                                    res.address,
                                );
                                resolve([res.address]);
                            } else if (COSMOS_REQUESTS.requestCosAccount === method) {
                                const pubKey = await walletController.getCosmosKeyDetails(
                                    params,
                                );
                                const chainDetail = currentWallet?.accounts.find(
                                    (acc) => acc.chainId === params.chainId,
                                ...
                                const pubKey = await walletController.getSuiKeyDetails();
                                sessionService.broadcastEvent(
                                    BROADCAST_REQUEST.connect,
                                    suiWalletDetail.address,
                                );
                    ...
                    .then(async (res) => {
                        if (res) {
                            ...
                            if (method === ETHEREUM_REQUESTS.personalSign) {
                                ...
                                privateMessage =
                                    providerController.ethSignTypedData(params);
                            } else if (method === ETHEREUM_REQUESTS.signTypedDataV1) {
                                privateMessage =
                                    providerController.ethSignTypedDataV1(params);
                            } else if (method === ETHEREUM_REQUESTS.signTypedDataV3) {
                                privateMessage =
                                    providerController.ethSignTypedDataV3(params);
                            } else if (method === ETHEREUM_REQUESTS.signTypedDataV4) {
                                privateMessage =
                                    providerController.ethSignTypedDataV4(params);
                            } else if (method === SOLANA_REQUESTS.signMessage) {
                                privateMessage =
                                    solProviderController.signMessage(params);
                            } else if (method === COSMOS_REQUESTS.verifyArbitrary) {
                                const { signer, message, signature, publicKey, chainId } =
                                    params?.[0]?.data;
                                const customChains =
                                    await walletController.getCustomChains();
...
const switchChain = async ({ params, method, origin, name, icon }) => {
    ...
    const currentChain = await walletController.getCurrentSelectedChain(customChains);
    const currentWallet = await walletController.getCurrentWallet();
    return new Promise(async (resolve, reject) => {
        ...
        if (
            ...
            const existingEthChain = currentWallet.accounts.find((chain) =>
                true
            );
            if (!existingEthChain) {
                reject(
